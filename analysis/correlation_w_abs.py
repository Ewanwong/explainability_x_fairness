...
def compute_fairness_reliance_correlation_w_abs_attribution(fairness_scores, reliance_scores_dict):

    correlation_results = {}
    for key in RELIANCE_KEYS:
        reliance_sum = sum(score for score in reliance_scores_dict[key])
        if reliance_sum > 0:
            reliance_score = [abs(score) for score in reliance_scores_dict[key]]
        else:
            reliance_score = [-abs(score) for score in reliance_scores_dict[key]]
        corr = scipy.stats.pearsonr(fairness_scores, reliance_score)
...
def compute_bias_correlation(fairness_scores, attribution_results):
    ...
    correlation_results = compute_fairness_reliance_correlation_w_abs_attribution(fairness_scores, reliance_scores_dict)
    return correlation_results
...
def main(args):
    ...
    os.makedirs("/nethome/yifwang/fairness_x_explainability/new_fairness_explainability/analysis/correlation_w_abs", exist_ok=True)
    results_dir = os.path.join("/nethome/yifwang/fairness_x_explainability/new_fairness_explainability/analysis/correlation_w_abs", '/'.join(args.results_dir.split("/")[-2:]))
    correlation_results_dir = os.path.join(results_dir, "correlation")
