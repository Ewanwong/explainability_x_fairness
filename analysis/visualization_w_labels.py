...
def main(args):
    ...
    for method in methods:
        ...
        for group in groups:
            ...
            group_predictions = individual_fairness_results["Labels"][group]
            ...
            for aggregation in aggregations:
                ...
                assert [attr["label"] for attr in attribution_results] == group_predictions
        ...
        # make plots and save 
        os.makedirs("/nethome/yifwang/fairness_x_explainability/new_fairness_explainability/analysis/visualization_w_labels", exist_ok=True)
        results_dir = os.path.join("/nethome/yifwang/fairness_x_explainability/new_fairness_explainability/analysis/visualization_w_labels", '/'.join(args.results_dir.split("/")[-2:]))
        ...
        for aggregation in individual_fairness_scores.keys():
            ...
            for key in RELIANCE_KEYS:
                plot_path = os.path.join(visualization_dir, f"{method}_{aggregation}_{key}_correlation_visualization_w_labels.png")
                all_reliance_scores = []
                for k, v in individual_fairness_scores[aggregation].items():
