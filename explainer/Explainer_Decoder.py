import torch
from captum.attr import Saliency, DeepLift, GuidedBackprop, InputXGradient, IntegratedGradients, Occlusion, \
    ShapleyValueSampling, DeepLiftShap, GradientShap, KernelShap, FeatureAblation
...
def _offload_to_cpu(*tensors):
    """Detach, move to CPU, and return the moved tensors (same order)."""
    moved = []
    for t in tensors:
        if t is None:
            moved.append(None)
        else:
            moved.append(t.detach().to("cpu", non_blocking=True))
    # make sure the async copies finished before we reuse CUDA memory
...
class BaseExplainer:
    ...
    def explain_embeddings(self, prompts, labels, targets, raw_inputs, example_indices):
        ...
        if targets is not None:
            target_ids = [self.tokenizer(target, return_tensors='pt', add_special_tokens=False)['input_ids'][:, :1] for
                          target in targets]
        ...
        if raw_inputs is not None:
            ...
            raw_input_ids = self.tokenizer(raw_inputs, return_tensors='pt', padding=True, add_special_tokens=False)[
                'input_ids']
            raw_input_ids = raw_input_ids.to(self.device)
        else:
            raw_input_ids = None

        explanations = self._explain(input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids,
                                     labels=labels, target_ids=target_ids, raw_input_ids=raw_input_ids,
    ...
    def explain_tokens(self, prompts, labels, targets, raw_inputs, example_indices):
        ...
        explanations = self._explain(input_ids=input_ids, attention_mask=attention_mask, labels=labels,
                                     target_ids=target_ids, raw_input_ids=raw_input_ids,
                                     example_indices=example_indices)
        return explanations
    ...
    def explain_dataset(self, dataset):
        ...
        for batch in tqdm(data_loader):
            ...
            if 'label' in batch:
                ...
                labels = None
            explanations = self.explain(prompts=prompts, labels=labels, targets=targets, raw_inputs=raw_inputs,
                                        example_indices=example_indices)
...
class BcosExplainer(BaseExplainer):
    ...
    def _explain(self, input_ids, attention_mask, position_ids=None, labels=None, target_ids=None, raw_input_ids=None,
        ...
        for explained_target_ids in target_ids:
            ...
            for i in range(batch_size):
                ...
                if raw_input_ids is not None:
                    ...
                    raw_input_indexes = find_sublist_indexes(input_ids[i].detach().cpu().float().numpy().tolist(),
                                                             raw_input_ids[i].detach().cpu().float().numpy().tolist())
                    if len(raw_input_indexes) == 0:
                        print(
                            f"Warning: raw_input_ids not found in input_ids for example {example_indices[i]}, return the original input")
                        raw_input_ids = None
                ...
                saliency_ixg_L2 = torch.norm(attributions_ixg_all[i:i + 1], dim=-1, p=2).detach().cpu().float().numpy()[
                    0]
                ...
                if raw_input_ids is not None:
                    raw_tokens = self.tokenizer.convert_ids_to_tokens(
                        raw_input_ids[i].detach().cpu().float().numpy().tolist())
                    raw_token_ixg_L2 = [saliency_ixg_L2.tolist()[raw_input_index] for raw_input_index in
                                        raw_input_indexes]
                    raw_token_ixg_mean = [saliency_ixg_mean.tolist()[raw_input_index] for raw_input_index in
                                          raw_input_indexes]
    ...
    def explain(self, prompts, labels, targets, raw_inputs, example_indices):
        return self.explain_embeddings(prompts=prompts, labels=labels, targets=targets, raw_inputs=raw_inputs,
                                       example_indices=example_indices)
...
class AttentionExplainer(BaseExplainer):
    ...
    def _explain(self, input_ids, attention_mask, labels, target_ids, raw_input_ids, example_indices=None):
        ...
        for explained_target_ids in target_ids:
            ...
            for i in range(batch_size):
                ...
                if raw_input_ids is not None:
                    raw_tokens = self.tokenizer.convert_ids_to_tokens(
                        raw_input_ids[i].detach().cpu().float().numpy().tolist())
                    raw_token_attention = [raw_attention_attribution.tolist()[raw_input_index] for raw_input_index in
                                           raw_input_indexes]
                    raw_token_attention_rollout = [attention_rollout_attribution.tolist()[raw_input_index] for
                                                   raw_input_index in raw_input_indexes]
                    raw_token_attention_flow = [attention_flow_attribution.tolist()[raw_input_index] for raw_input_index
                                                in raw_input_indexes]
                    raw_tokens = [token for token in raw_tokens if token != self.tokenizer.pad_token]
        ...
        attention_explanations = {"raw_attention": all_raw_attention_explanations,
                                  "attention_rollout": all_attention_rollout_explanations,
                                  "attention_flow": all_attention_flow_explanations}
...
class GradientNPropabationExplainer(BaseExplainer):
    ...
    def _explain(self, input_ids, attention_mask, position_ids=None, labels=None, target_ids=None, raw_input_ids=None,
        ...
        prediction_probabilities = probabilities[
            torch.arange(probabilities.shape[0]), predicted_ids.squeeze(1)].unsqueeze(1)  # shape: [batch_size, 1]
        ...
        for explained_target_ids in target_ids:
            ...
            target_probabilities = probabilities[
                torch.arange(probabilities.shape[0]), explained_target_ids.squeeze(1)].unsqueeze(
                1)  # shape: [batch_size, 1]
            attributions_cpu = None
            if self.method == 'Saliency':
                ...
                if self.method == 'IntegratedGradients':
                    ...
                    try:
                        ...
                        attributions_cpu, = _offload_to_cpu(attributions)
                        del attributions
                        try:
                            del embeddings
                        except NameError:
            ...
            attributions_all = attributions_cpu if attributions_cpu is not None else attributions
            # clear cache
            empty_all_cuda_caches()
            ...
            for i in range(batch_size):
                ...
                if raw_input_ids is not None:
                    raw_token_saliency_L2 = [saliency_L2.tolist()[raw_input_index] for raw_input_index in
                                             raw_input_indexes]
                    raw_token_saliency_mean = [saliency_mean.tolist()[raw_input_index] for raw_input_index in
                                               raw_input_indexes]
                    raw_tokens = [token for token in raw_tokens if token != self.tokenizer.pad_token]
                    # Collect results for the current example and class
                # skip padding tokens
                ...
                if raw_input_ids is not None:
                    ...
                    all_saliency_L2_results[i].append(result_L2)
                    all_saliency_mean_results[i].append(result_mean)
        saliency_results = {f"{self.method}_L2": all_saliency_L2_results,
                            f"{self.method}_mean": all_saliency_mean_results}
        return saliency_results
    ...
    def _explain(self, input_ids, attention_mask, position_ids=None, labels=None, target_ids=None, raw_input_ids=None,
        ...
        for explained_target_ids in target_ids:
            ...
            for i in range(batch_size):
                ...
                if raw_input_ids is not None:
                    raw_token_attribution_i = [attribution_i.tolist()[raw_input_index] for raw_input_index in
                                               raw_input_indexes]
                    raw_tokens = [token for token in raw_tokens if token != self.tokenizer.pad_token]
                    # Collect results for the current example and class
                # skip padding tokens
