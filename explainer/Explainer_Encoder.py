...
from utils.lime_utils import explain
...
class BertModelProbWrapper(torch.nn.Module):
    def __init__(self, model):
        super(BertModelProbWrapper, self).__init__()
        self.model = model
...
class OcclusionExplainer(BaseExplainer):
    def __init__(self, model, tokenizer, method='Occlusion', baseline='zero'):
        self.model = BertModelProbWrapper(model)
        self.model.eval()
        self.model.to(model.device)
        self.tokenizer = tokenizer
